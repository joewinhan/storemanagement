...
$(function(){
	$('#approval_in_table').datagrid({    
	    url:'page/showPurchaseOrderStatusEq2',    
	    columns:[[    
	    ...
	    toolbar: [
			...
			*/
			...
			text:'提交入库申请',
			handler: function(){
				if($("#approval_in_table").datagrid('getSelections').length != 0){
    				$.messager.confirm('确认','您确认要提交入库申请吗?',function(r){    
	    			    if (r){    
	    			    	var arr = $('#approval_in_table').datagrid("getSelections")
			    			var ids = "";
			    			for(var i in arr){
			    				if(i == arr.length-1){
			    					ids += arr[i].id;
			    			...
			    			$.getJSON("page/modifyPurchaseOrderStatus",{ id: ids,status:3}, function(data){
								if(data.status == 200){
									alert(data.msg);
									$('#approval_in_table').datagrid('reload');
								}
			    			});  
	    			    }    
	    			});
...
<table id="approval_in_table"></table> 
</body>
</html>