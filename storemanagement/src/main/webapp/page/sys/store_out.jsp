...
$(function(){
	$('#store_out_table').datagrid({    
	    url:'page/showSaleOrderByStatus?status=3',    
	    columns:[[    
	    ...
	    toolbar: [
			...
			*/
			...
			text:'允许出库',
			handler: function(){
				if($("#store_out_table").datagrid('getSelections').length != 0){
    				$.messager.confirm('确认','您确认要批准出库吗?',function(r){    
	    			    if (r){    
	    			    	var arr = $('#store_out_table').datagrid("getSelections")
			    			var ids = "";
	    			    	var amounts="";
	    			    	var cids="";
			    			for(var i in arr){
			    			...
			    			$.getJSON("page/modifySaleOrderStatus",{ id: ids,status:4,amount:amounts,cid:cids}, function(data){
								if(data.status == 200){
									alert(data.msg);
									$('#store_out_table').datagrid('reload');
								}
			    			});  
	    			    }    
	    			});
...
<table id="store_out_table"></table> 
</body>
</html>