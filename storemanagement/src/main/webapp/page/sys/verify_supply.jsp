...
$(function(){
	$('#approval_supply_table').datagrid({    
	    url:'page/showPurchaseOrderStatusEq1',    
	    columns:[[    
	    ...
	    toolbar: [{
			...
			text:'通过申请',
			handler: function(){
				if($("#approval_supply_table").datagrid('getSelections').length != 0){
    				$.messager.confirm('确认','您确认要通过选中的采购申请吗?',function(r){    
	    			    if (r){    
	    			    	var arr = $('#approval_supply_table').datagrid("getSelections")
			    			var ids = "";
			    			for(var i in arr){
			    				if(i == arr.length-1){
			    					ids += arr[i].id;
			    			...
			    			$.getJSON("page/modifyPurchaseOrderStatus",{ id: ids,status:2}, function(data){
								if(data.status == 200){
									alert(data.msg);
									$('#approval_supply_table').datagrid('reload');
								}
			    			});  
	    			    }    
	    			});
			...
			handler: function(){
				if($("#approval_supply_table").datagrid('getSelections').length != 0){
    				$.messager.confirm('确认','您确认要拒绝选中的采购申请吗?',function(r){    
	    			    if (r){    
	    			    	var arr = $('#approval_supply_table').datagrid("getSelections")
			    			var ids = "";
			    			for(var i in arr){
...
<table id="approval_supply_table"></table> 
</body>
</html>